...
const RecurringDepositProducts = () => {
  const [products, setProducts] = useState<GetRecurringDepositProductsResponse[]>([]);
    ...
    <div className="min-h-screen px-6 py-10 max-w-7xl mx-auto text-[15px]">
      <AppBreadCrumbs
        items={[
          ...
          { label: "Recurring Deposit Products", current: true },
        ]}
      />
      ...
      <div className="mb-6">
        <Button
          ...
          onClick={() => navigate("/products/recurring-deposit-products/create")}
        >
          <Plus className="mr-2" /> Create Recurring Deposit Product
        </Button>
      </div>
          ...
          <TableBody>
            {paginated.map((p) => (
              <TableRow
                ...
                onClick={() =>
                  navigate(`/products/recurring-deposit-products/${p.id}/general`)
                }
              >
            ...
            {paginated.length === 0 && (
              <TableRow>
                <TableCell colSpan={2} className="px-6 py-6 text-center text-zinc-500">
                  No results.
                </TableCell>
              </TableRow>
            )}
