...
const Configurations = () => {
  ...
  const handleEdit = async (row: GlobalConfigurationPropertyData) => {
    ...
    const next = window.prompt(`Set ${row.name} ${hasString ? "string" : "numeric"} value:`, current);
    if (next == null) return;
          ...
          <TableBody>
            {rows.map((row) => (
              <TableRow key={row.name}>
                <TableCell className="px-6 py-3 font-medium">{row.name}</TableCell>
                ...
                <TableCell className="px-6 py-3">
                  <div className="flex items-center gap-3">
                    ...
                    />
                    <span className={row.enabled ? "text-green-600" : "text-rose-500"}>
                ...
                <TableCell className="px-6 py-3">{row.stringValue ?? ""}</TableCell>
                <TableCell className="px-6 py-3">{row.dateValue ?? ""}</TableCell>
