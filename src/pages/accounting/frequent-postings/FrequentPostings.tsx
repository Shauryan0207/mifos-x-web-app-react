...
import { AccountingRulesApi, CurrencyApi, JournalEntriesApi, OfficesApi, PaymentTypeApi, type CurrencyData, type GetOfficesResponse, type GetPaymentTypeData } from "@/fineract-api";
...
const FrequentPostings = () => {
    ...
    useEffect(() => {
        const fetchDropdowns = async () => {
            try {
                const [officesRes, currenciesRes, paymentTypesRes, accountingRulesRes] = await Promise.all([
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!formData.office || !formData.currency || !formData.date || !selectedRule) {
        ...
        const payload = {
            ...
            paymentTypeId: formData.paymentType ? parseInt(formData.paymentType) : undefined,
                ...
                <form className="space-y-6" onSubmit={handleSubmit}>
                        ...
                        <>
                            <div className="flex flex-wrap gap-6">
                                ...
                                <div className="w-full md:w-[48%] space-y-2">
                                    ...
                                    <Input value={debitAmount} onChange={e => setDebitAmount(e.target.value)} placeholder="Enter amount" />
                                </div>
                            </div>
                            ...
                            <div className="flex flex-wrap gap-6">
                                ...
                                <div className="w-full md:w-[48%] space-y-2">
                                    ...
                                    <Input value={creditAmount} onChange={e => setCreditAmount(e.target.value)} placeholder="Enter amount" />
                                </div>
                            </div>
                        </>
                    )}
                    ...
                    <div className="flex flex-wrap gap-6">
                        <div className="w-full md:w-[48%] space-y-2">
                            ...
                            <Input value={formData.refNo} onChange={e => setFormData(prev => ({ ...prev, refNo: e.target.value }))} className="w-full" placeholder="Enter reference number" />
                        </div>
                        <div className="w-full md:w-[48%] space-y-2">
                            <Label>Transaction Date *</Label>
                            <Input type="date" value={formData.date} onChange={e => setFormData(prev => ({ ...prev, date: e.target.value }))} className="w-full" />
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-6">
                        ...
                        <div className="w-full md:w-[48%] space-y-2">
                            ...
                            <Input value={formData.accNo} onChange={e => setFormData(prev => ({ ...prev, accNo: e.target.value }))} className="w-full" placeholder="Enter account number" />
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-6">
                        <div className="w-full md:w-[48%] space-y-2">
                            ...
                            <Input value={formData.chequeNo} onChange={e => setFormData(prev => ({ ...prev, chequeNo: e.target.value }))} className="w-full" placeholder="Enter cheque number" />
                        </div>
                        <div className="w-full md:w-[48%] space-y-2">
                            <Label>Routing Code</Label>
                            <Input value={formData.routingCode} onChange={e => setFormData(prev => ({ ...prev, routingCode: e.target.value }))} className="w-full" placeholder="Enter routing code" />
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-6">
                        <div className="w-full md:w-[48%] space-y-2">
                            ...
                            <Input value={formData.receiptNo} onChange={e => setFormData(prev => ({ ...prev, receiptNo: e.target.value }))} className="w-full" placeholder="Enter receipt number" />
                        </div>
                        <div className="w-full md:w-[48%] space-y-2">
                            <Label>Bank Number</Label>
                            <Input value={formData.bankNo} onChange={e => setFormData(prev => ({ ...prev, bankNo: e.target.value }))} className="w-full" placeholder="Enter bank number" />
                        </div>
                    </div>

                    <div className="space-y-2">
                        <Label>Comments</Label>
                        <Input value={formData.comments} onChange={e => setFormData(prev => ({ ...prev, comments: e.target.value }))} className="w-full" placeholder="Enter comments" />
                    </div>

                    <div className="flex justify-end gap-4 pt-6">
                        <Button type="button" variant="outline" onClick={() => navigate('/accounting')}>Cancel</Button>
                        <Button type="submit" className="bg-[#1074b9] hover:bg-[#1074c9] text-white">Submit</Button>
                    </div>
                </form>
            </div>
        </div>
