...
import { GeneralLedgerAccountApi, type GetGLAccountsResponse, type PutGLAccountsRequest } from "@/fineract-api";
import { getConfiguration } from "@/lib/fineract-openapi";

import { Plus } from "lucide-react";
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faLock, faLockOpen, faPenToSquare, faTrash } from '@fortawesome/free-solid-svg-icons';
        ...
        <div className="min-h-screen px-6 py-10 bg-gray-50 dark:bg-zinc-900">
            ...
            <div className="bg-white dark:bg-zinc-800 shadow-md rounded-lg p-8 max-w-2xl mx-auto">
                <div className="flex justify-between">
                    ...
                    <div className="flex gap-4 mb-6">
                        <Button
                            ...
                            onClick={() => navigate(`/accounting/chart-of-accounts/gl-accounts/view/${account.id}/edit`)}
                    ...
                    {account.usage?.value === "HEADER" && (
                        ...
                        <div>
                            <Button
                                className="bg-[#1074b9] hover:bg-[#1074c9] text-white cursor-pointer"
                                onClick={() => navigate(`/accounting/chart-of-accounts/gl-accounts/create?parent=${account.id}&accountType=${account.type?.id}`)}
                            >
                                <Plus />Subledger Account
                            </Button>
                        </div>
                    )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-y-5 text-sm text-zinc-700 dark:text-zinc-200">
                    ...
                    <div className="text-zinc-600 dark:text-zinc-400">{account.type?.value}</div>

                    <div className="font-medium">GL Code</div>
                    <div className="text-zinc-600 dark:text-zinc-400">{account.glCode}</div>

                    <div className="font-medium">Account Usage</div>
                    <div className="text-zinc-600 dark:text-zinc-400">{account.usage?.value}</div>

                    <div className="font-medium">Manual Entries Allowed</div>
                    <div className="text-zinc-600 dark:text-zinc-400">{account.manualEntriesAllowed ? 'Yes' : 'No'}</div>
                </div>
