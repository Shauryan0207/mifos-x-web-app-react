...
function buildSavingsMenu(
  ...
  const isPending  = !!(
    ...
    String(status.value || "").toLowerCase().includes("pending")
  );
  const isApproved = !!(
  ...
  if (isApproved) {
    return [
      ...
      { label: "Add Charge", path: link("Add Charge") },
      { label: "More", children: [{ label: "Assign Staff", path: link("Assign Staff") }] },
    ];
  }
  ...
  return [{ label: "More", children: [{ label: "Assign Staff", path: link("Assign Staff") }] }];
}
...
const SavingsAccountView = () => {
  ...
  const isApproved = Boolean((s.approved && !s.active) || s.waitingForActivation || statusVal === "approved");
          ...
          <div className="space-y-2">
            ...
            {!(s.rejected || s.submittedAndPendingApproval) && (
              <div className="mt-3 grid max-w-md grid-cols-2 text-sm">
                ...
                <div className="text-right">{(acct as any)?.summary?.accountBalance}</div>
                <div>Available Balance</div>
                <div className="text-right">{(acct as any)?.summary?.availableBalance}</div>
              </div>
            )}
          </div>
