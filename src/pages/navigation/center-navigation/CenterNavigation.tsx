...
import { CentersApi, type CenterData, type GetCentersCenterIdResponse, type GetCentersTemplateResponse, type GetOfficesResponse } from "@/fineract-api";
...
const CenterNavigation = ({ centerId }: CenterProps) => {
  ...
  const [centerStatus, setCenterStatus] = useState<GetCentersCenterIdResponse | null>(null);
  const [template, setTemplate] = useState<GetCentersTemplateResponse | null>(null)
  const [centerSummary, setCenterSummary] = useState<CenterSummaryDetails | null>(null);
  ...
  return (
    <div className="space-y-6 text-sm text-gray-700 dark:text-gray-300">
      ...
      <div className="flex items-center gap-4">
        <FontAwesomeIcon icon={faBuilding} size="2x" className="text-gray-700 dark:text-gray-200" />
        <div>
          ...
          <p className="text-gray-500">
            Account No: <span className="font-medium">{centerDetails.accountNo}</span>
          </p>
          {office?.externalId && (
            <p className="text-gray-500">
              External ID: <span className="font-medium">{office?.externalId}</span>
            </p>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-y-3">
        ...
        {centerSummary && (
          <>
            ...
            <div className="font-medium">Number of Active Client Borrowers:</div>
