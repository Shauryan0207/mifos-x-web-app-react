...
import { StaffApi, UsersApi, type GetUsersResponse, type GetUsersTemplateResponse, type StaffData } from "@/fineract-api";
...
const EditUsers = () => {
    ...
    // fetch template
    useEffect(() => {
        userApi.template22().then(res => setUsers(res.data)).catch(console.error);
    }, []);

    // fetch staff list when office changes
    useEffect(() => {
        if (!formData.office) return;
        staffApi.retrieveAll16().then(res => setStaff(res.data || [])).catch(console.error);
    }, [formData.office]);

    // fetch user details for editing
    useEffect(() => {
        if (!id) return;
        userApi.retrieveOne31(Number(id)).then(res => {
            const data = res.data;
            setUser(data);
            setFormData(prev => ({
                ...prev,
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // basic validation
        if (!formData.username || !formData.email || !formData.firstName || !formData.lastName || formData.office || formData.roles) {
                ...
                <form className="space-y-6" onSubmit={handleSubmit}>
                    <div className="flex flex-wrap gap-6">
                        <div className="w-full md:w-[48%] space-y-2">
                            ...
                            <Input placeholder="Enter username" className="w-full" value={formData.username} />
                        </div>
                        <div className="w-full md:w-[48%] space-y-2">
                            <Label>Email *</Label>
                            <Input placeholder="Enter email" className="w-full" value={formData.email} />
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-6">
                        <div className="w-full md:w-[48%] space-y-2">
                            ...
                            <Input placeholder="Enter First Name" className="w-full" value={formData.firstName} />
                        </div>
                        <div className="w-full md:w-[48%] space-y-2">
                            <Label>Last Name *</Label>
                            <Input placeholder="Enter Last Name" className="w-full" value={formData.lastName} />
                        </div>
                    </div>

                    <div className="flex flex-wrap gap-6">
                        <div className="w-full md:w-[48%] flex items-center gap-2">
                            <Checkbox id="manual-entries-1" checked={formData.passwordNeverExpiers} />
